...
jianpu-ly（来自 [Silas S. Brown](https://ssb22.user.srcf.net/mwrhome/jianpu-ly.html) ）是一个 Python 程序（兼容 Python 2 和 Python 3 ），用于在 Lilypond 中打印简谱。当然，你也可以通过 jianpu-ly 同时打印简谱和线谱。
...
Lilypond头部代码： `LPH: (定义) :LPH` （每个分隔符必须位于各行行首）
